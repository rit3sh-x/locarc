...
export function MapCursorProvider({
    children,
}: {
    ...
    return (
        <MapCursorContext.Provider value={{ pos, setPos }}>
            {children}
        </MapCursorContext.Provider>
    );
}
...
export function useMapCursor() {
    ...
    if (!ctx)
        throw new Error("useMapCursor must be used inside MapCursorProvider");
    return ctx;
}
